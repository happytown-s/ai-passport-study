...
const tbFiles = [
  'textbook-ai-basics.json',
  'textbook-ml-basics.json',
  'textbook-generative-ai.json',
  'textbook-prompt-engineering.json',
  'textbook-ai-risks.json',
  'textbook-legal.json',
  'textbook-business.json',
];
...
for (const file of tbFiles) {
  ...
  for (const topic of data) {
    for (const kw of topic.keywords) {
      totalKeywords++;
      // Check if keyword exists in terms (exact or partial match)
      ...
      for (const t of terms) {
        ...
        // Partial match: keyword is contained in term or vice versa
        if (t.term.toLowerCase().includes(kwLower) || kwLower.includes(t.term.toLowerCase())) {
          if (kwLower.length >= 3) { // Only count if meaningful overlap
            found = true;
            break;
          }
        }
